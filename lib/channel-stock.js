...
const { variantTargets } = require('./variant-alias');
...
// 이 줄이 카페24에서 담당하는 옵션들 중 실제로 수량을 보낼 수 있는 것 (재고관리 켜져 있고 살아 있는 옵션)
function pushTargets(inv) {
  return variantTargets(inv).filter(t => t.active && t.cafe24StockTracked === true);
}
...
function planCafe24Push(db, policy) {
  ...
  for (const inv of ((db && db.inventory) || [])) {
    if (!inv.productNo || !inv.variantCode) { skipped.noVariant++; continue; }
    const targets = pushTargets(inv);
    if (!targets.length) { if (inv.cafe24VariantActive === false) skipped.noVariant++; else skipped.notTracked++; continue; }
    const sku = skuOf(inv);
    const available = availableQty(inv, reserved.get(sku) || 0, policy);
    if (available === null) { if (inventoryCountKnown(inv) && !rowVerified(inv)) skipped.unverified++; else skipped.needsCount++; continue; }
    for (const target of targets) {
      const cafe24Qty = cafe24QtyOf(target);
      if (available === cafe24Qty) { skipped.same++; continue; }
      rows.push({
        id: inv.id, sku, name: target.alias ? target.name || inv.name || '' : inv.name || '', color: inv.color || '', size: inv.size || '',
        productNo: target.productNo, variantCode: target.variantCode, alias: !!target.alias,
...
function clearSettledDirty(db, policy) {
  ...
  for (const inv of ((db && db.inventory) || [])) {
    ...
    const targets = inv.productNo && inv.variantCode ? pushTargets(inv) : [];
    const available = availableQty(inv, reserved.get(skuOf(inv)) || 0, policy);
    if (!targets.length || available === null || targets.every(t => available === cafe24QtyOf(t))) { delete inv.channelDirty; cleared++; }
  }
  return cleared;
}
...
async function pushCafe24Stock(db, rows, opts = {}) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (!inv) continue;
    // row.variantCode 가 대표와 다르면 alias 옵션(밀이 마켓 등)에 보내는 것 — 수량 기록도 그 alias 에 남긴다
    const target = row.variantCode && String(row.variantCode) !== String(inv.variantCode)
      ? (inv.aliases || []).find(a => String(a.variantCode) === String(row.variantCode))
      : inv;
    if (!target) continue;
    if (i > 0) await wait(PUSH_INTERVAL_MS);
    const qty = Math.max(0, Number(row.available) || 0);
    const from = cafe24QtyOf(target);
    let res = null;
    let error = '';
    try { res = await send({ id: inv.id, productNo: target.productNo, variantCode: target.variantCode }, qty); }
    ...
    if (error) {
      ...
      target.cafe24Qty = qty;
      target.cafe24PushedAt = at;
      target.cafe24PushedQty = qty;
      if (target === inv) { inv.cafe24PushedAt = at; inv.cafe24PushedQty = qty; }
      // 대표+alias 전부 맞춰졌을 때만 '반영 대기' 를 내린다
      if (pushTargets(inv).every(t => cafe24QtyOf(t) === qty)) delete inv.channelDirty;
      delete inv.channelSyncError;
      delete inv.channelSyncFails;
      out.pushed++;
    }
    logChannelSync(db, {
      ts: at, channel: 'cafe24', sku: skuOf(inv), name: [target === inv ? inv.name : target.name || inv.name, inv.color, inv.size].filter(Boolean).join(' '),
