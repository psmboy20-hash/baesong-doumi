...
// 카페24 재고를 0으로 밀어버리는 사고 방지 — 실물 수량을 한 번이라도 채운 뒤에만 자동 반영을 켤 수 있다
function stockInitialized(db) {
  if (Array.isArray(db && db.stocktakes) && db.stocktakes.length > 0) return true;
  return ((db && db.inventory) || []).reduce((sum, inv) => sum + (Number(inv.qty) || 0), 0) > 0;
}

// 대기·접수중 항목이 잡아둔 수량 (항목→재고 줄 매칭은 출고 차감과 같은 규칙)
function reservedBySku(db) {
...
function availableQty(inv, reserved, policy) {
  if (!inv || inv.retiredAggregate || !inventoryCountKnown(inv)) return null; // 실사 필요·집계 줄은 반영 대상 아님
...
function planCafe24Push(db, policy) {
  const reserved = reservedBySku(db);
  const rows = [];
  const skipped = { noVariant: 0, notTracked: 0, needsCount: 0, same: 0 };
  for (const inv of ((db && db.inventory) || [])) {
    ...
    const sku = skuOf(inv);
    const available = availableQty(inv, reserved.get(sku) || 0, policy);
    if (available === null) { skipped.needsCount++; continue; }
