...
// 시트에서 '패키지'(또는 ⭐·★·박스·box)라고 적혀 있으면 박스(패키지)포장, 그 밖('일반'·빈칸·•)은 일반포장
function normalizeSeedingPackMark(value) {
  return /패키지|⭐|★|☆|박스|box/i.test(compact(value)) ? 'box' : 'plain';
}
...
function seedingProductFields(row, col) {
  ...
  const manualProduct = value(col.product);
  const manualColor = value(col.color);
  const manualSize = value(col.size);
  const masterProduct = value(col.masterProduct);
  const masterColor = value(col.masterColor);
  const masterSize = value(col.masterSize);
  const selectedOption = value(col.selectedOption);
  const selectedParts = selectedOption.split('|').map(compact);
  const fallbackProduct = masterProduct || selectedParts[0] || '';
  const fallbackColor = masterColor || selectedParts[1] || '';
  const fallbackSize = masterSize || selectedParts[2] || '';
  const hasManualInput = !!(manualProduct || manualColor || manualSize);
  const identityMatchesManual = hasManualInput && !!manualProduct && !!masterProduct &&
    sameText(manualProduct, masterProduct) &&
    sameText(manualColor, masterColor) &&
    sameText(manualSize, masterSize);
  const useInternalIdentity = !hasManualInput || identityMatchesManual;
  const fields = {
    product: hasManualInput ? manualProduct : fallbackProduct,
    size: hasManualInput ? manualSize : fallbackSize
  };
  if (col.color >= 0 || col.masterColor >= 0) fields.color = hasManualInput ? manualColor : fallbackColor;
  if (col.selectedOption >= 0) fields.selectedOption = useInternalIdentity ? selectedOption : '';
  if (col.masterProductNo >= 0) fields.productNo = useInternalIdentity ? value(col.masterProductNo) || null : null;
  if (col.variantCode >= 0) fields.variantCode = useInternalIdentity ? value(col.variantCode) : '';
  return fields;
}
  ...
  for (const source of rows) {
    ...
    if (!existing) {
      const item = Object.assign({}, source, {
        ...
        packMark: normalizeSeedingPackMark(source.packMark),
        id: db.nextId++,
    ...
    changed = setField(existing, 'packMark', normalizeSeedingPackMark(source.packMark)) || changed;
