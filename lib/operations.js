...
// 우체국 처리상태 코드 — 03(수거됨)·05(취소됨)이면 더 손댈 일이 없다. 그 밖(00·01·02·04)은 아직 진행 중.
const EPOST_DONE_CODES = ['03', '05'];
// 접수 취소가 가능한 상태 — 04(수거 안 됨)는 기사님이 이미 다녀간 뒤라 취소 대상이 아니다.
const EPOST_CANCELABLE_CODES = ['00', '01', '02'];

// 우체국 처리상태 코드 정규화 — 실측(2026-09-07): 집하 뒤 배송중·배달완료 건은 treatStusCd=06 으로 돌아온다.
// 앱 상태표(00~05)에 없어 '운송장 출력 대기'로 남던 문제 → 03(수거됨)과 같게 본다.
function normalizeEpostStus(code) {
  const c = String(code || '').trim();
  return c === '06' ? '03' : c;
}
function epostTreatmentStatus(xml) {
  const match = String(xml || '').match(/<treatStusCd>\s*(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?\s*<\/treatStusCd>/i);
  return match ? normalizeEpostStus(match[1]) : '';
}
...
function prepareEpostCancellation(db, type, id, at) {
  ...
  for (const entry of entries) {
    // 취소 뒤 다시 접수한 건에는 예전 취소 기록(다른 orderNo)이 남아 있다 — 그걸 '이미 끝난 취소'로 오해하면
    // 새 접수는 그대로 둔 채 카페24 배송등록만 지우는 사고가 난다(2026-09-15 이유정 건). 같은 접수의 기록만 이어받는다.
    const previous = entry.item.epostCancelOp || {};
    const existing = previous.orderNo === orderNo ? previous : {};
...
function flagMissingCanceledSheetSource(item, at) {
  ...
  const message = '시트 행 고유번호가 없어 취소한 송장에 취소선을 긋지 못했어요. 구글시트에서 이 송장번호에 직접 취소 표시를 해 주세요.';
...
function finalizeEpostCancellation(db, orderNo, restoreStock, at) {
  ...
  // 이미 앱 정리까지 끝난(local_finalized) 줄은 다시 만지지 않는다 — 재접수된 살아 있는 배송등록을 지우면 안 된다
  for (const entry of active.filter(row => row.type === 'order' && row.item.cafe24Shipped && row.item.epostCancelOp.state !== 'local_finalized')) {
  ...
  for (const rows of cafe24Groups.values()) {
    ...
    // 옛 송장 삭제는 canceledShipment 참조로 추적하고, 플래그는 즉시 내린다 —
    // 삭제가 실패한 채 재접수돼도 새 송장이 카페24에 다시 등록되도록
    for (const row of rows) { row.canceledShipment = ref; row.cafe24Shipped = false; }
  }
  ...
  for (const entry of active) {
    ...
    if (item.epostCancelOp.state !== 'local_finalized') {
      if (entry.type === 'seeding' && item.sheetWritten) {
        ...
        item.sheetWritten = false; // 시트 삭제 실패 후 재접수돼도 새 송장이 시트에 다시 기록되도록
...
// 카페24가 아닌 판매채널(29CM·무신사·GS샵 등) 주문은 카페24에 송장을 등록할 수 없다 — 헛된 조회·오류 표시를 막는다
const NON_CAFE24_CHANNELS = ['29cm', 'musinsa', 'gsshop', 'other'];
...
function groupCafe24ShipmentItems(matchedItems) {
  ...
  for (const entry of (matchedItems || [])) {
    ...
    if (NON_CAFE24_CHANNELS.includes(String(item.sourceChannel || ''))) continue;
...
function stockMatchesByName(inv, item) {
...
function selectStockMatches(inventory, item, matchByName = stockMatchesByName) {
  const rows = Array.isArray(inventory) ? inventory : [];
  const unique = matches => matches.length === 1 ? matches : [];
  if (item.variantCode) {
    const code = String(item.variantCode);
    // 같은 실물의 다른 카페24 옵션(예: 밀이 마켓 상품)은 대표 줄의 aliases 에 들어 있다
    return unique(rows.filter(inv => String(inv.variantCode || '') === code ||
      (Array.isArray(inv.aliases) && inv.aliases.some(a => String(a.variantCode) === code))));
  }
...
// 입출고 장부의 ref 는 원칙적으로 코드(RMA-/SEED-/SHIP-/PACK-)만 남기지만,
// 기초재고·실사처럼 고객과 무관한 시스템 메모는 그대로 둔다 (수불부에서 근거로 쓰임).
// 사람이 적은 메모는 ref 가 아니라 row.note 로 들어가므로 여기서는 고정된 두 형태만 통과시킨다.
const STOCK_REF_NOTE = /^(?:카페24 수량 기준|실사 \d{4}-\d{2}-\d{2})$/;
...
function ensureOperationalFields(db) {
  ...
  for (const row of (db.stockLog || [])) {
    // INB-<번호>: 입고 예정에서 들어온 본사 입고 — 사람 이름이 아니라 시스템 번호라 지우지 않는다
    if (!row.ref || /^(?:RMA|SEED|SHIP|PACK|INB)-[A-Z0-9-]+$/i.test(String(row.ref)) || /^\d{8}-\d+$/.test(String(row.ref))) continue;
    if (STOCK_REF_NOTE.test(String(row.ref))) continue; // 사람 이름이 아닌 시스템 메모(기초재고·실사)는 남긴다
    row.ref = '';
    changed = true;
  }
  return changed;
...
// 배달 조회 순서: 한 번도 안 본 것 → 가장 오래전에 본 것 순 (앞 10건만 보다가 뒤 건이 영영 밀리는 일 방지)
function deliveryCheckOrder(items) {
  return [...(items || [])].sort((a, b) => {
    const ta = a.deliveryCheckedAt ? Date.parse(a.deliveryCheckedAt) || 0 : 0;
    const tb = b.deliveryCheckedAt ? Date.parse(b.deliveryCheckedAt) || 0 : 0;
    return ta - tb;
  });
}

// 한 포장(같은 주문·합포장) 안의 받는 사람·연락처·주소가 서로 다르면 접수 전에 막는다
// (합포장 뒤 카페24에서 주소가 바뀌는 등 — 합포 시점 검사만으로는 못 잡는 경우)
function groupRecipientConflict(items, recipientKey) {
  const keys = new Set((items || []).map(item => String(recipientKey(item) || '')));
  if (keys.size <= 1) return '';
  return '묶인 상품들의 받는 사람·연락처·주소가 서로 달라요. 합포장을 풀거나 주소를 확인한 뒤 다시 접수해 주세요.';
}
