...
function expandSelectedFulfillments(db, selected) {
  ...
  const allowedTypes = new Set((selected || []).map(sel => sel.type === 'seeding' ? 'seeding' : 'order'));
  const keys = [];
  ...
  for (const key of keys) {
    for (const entry of all) {
      if (!allowedTypes.has(entry.type) || fulfillmentKey(entry.type, entry.item) !== key) continue;
...
function markPrintedFulfillments(db, selected, printedAt) {
  ...
  const selectedEntries = expandSelectedFulfillments(db, selected)
    ...
    .filter(entry => entry && entry.item.epost && entry.item.epost.label);
  ...
  let items = 0;
  for (const entry of all) {
    if (!entry.item.epost || !entry.item.epost.label) continue;
    const orderNo = String(entry.item.epost.orderNo || '').trim();
...
function resolvePackingMergeSelection(db, selected) {
  ...
  const invalid = item => {
    if (item.shippingHold || item.parcelSplitId) return '분리배송·재고 기다림 품목이 있는 주문은 합포할 수 없어요.';
