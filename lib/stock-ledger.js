...
const { reservedBySku } = require('./channel-stock');
...
function initFromCafe24(db, opts = {}) {
  ...
  const rows = [];
  let skipped = 0;
  // 카페24 판매가능 수량은 "아직 안 보낸 카페24 주문"만큼 이미 빠진 숫자 → 실물은 그만큼 더 있다 (안 더하면 채널 반영 때 이중 차감)
  const pendingCafe24 = opts.pendingCafe24 || reservedBySku(db, { channels: ['cafe24'] });
  for (const inv of (db.inventory || [])) {
    ...
    const next = Math.max(0, Math.round(Number(inv.cafe24Qty)) + (pendingCafe24.get(skuOf(inv)) || 0));
    inv.qty = next;
    inv.needsCount = false;
    inv.stockInitAt = now.toISOString(); // 카페24 수량을 기준으로 채운 시각 — 채널 반영 대상이 되는 표식
