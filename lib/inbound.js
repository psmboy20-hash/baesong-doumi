...
// 오늘부터 days 일 안에 들어올 예정 건 (홈 '오늘 할 일'·재고 탭 배지용)
function expectedSoon(db, today, days) {
  const span = Number.isFinite(Number(days)) ? Number(days) : 3;
  const base = String(today || dateOf(new Date()));
  const until = new Date(base + 'T00:00:00');
  until.setDate(until.getDate() + span);
  const limit = dateOf(until);
  return inboundList(db).filter(row => row.status === 'expected' && row.eta && row.eta <= limit);
}
