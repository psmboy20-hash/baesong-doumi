...
- 현재 XLSX 런타임은 `xlsx 0.18.5`; 교체 전까지 크기·형식 제한으로 노출을 줄인다.

## Commands

- Focused tests: `node --test tests/hardening.test.js`
- Full tests: `npm test`
- Syntax: `node --check server.js && node --check public/app.js && node --check public/label-logic.js`
...
- `public/label-logic.js`: 브라우저와 테스트가 함께 쓰는 라벨 선택·표시 로직
...
- 배포 전 전체 63개 이상의 기존 테스트와 새 테스트를 모두 통과시킨다.
...
- `xlsx` 교체는 새 의존성 검토 후 별도 증분으로 수행한다.
